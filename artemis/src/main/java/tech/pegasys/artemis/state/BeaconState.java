  ...
  */
  ...
  private int min_empty_validator_index(ArrayList<ValidatorRecord> validators, int current_slot) {
    for (int i = 0; i < validators.size(); i++) {
      ...
      if (v.balance == 0 && v.latest_status_change_slot.getValue() + ZERO_BALANCE_VALIDATOR_TTL
          <= current_slot) {
        return i;
      }
    }
  ...
  private int process_deposit(BeaconState state, int pubkey, int deposit, Bytes32 proof_of_possession,
    ...
    for (int i=0; i < validator_pubkeys.length; i++) {
      validator_pubkeys[i] = state.validator_registry.get(i).pubkey;
    }

    int index = -1;

    if (!Arrays.asList(validator_pubkeys).contains(pubkey)) {
      // Add new validator
      ...
      index = Arrays.asList(validator_pubkeys).indexOf(pubkey);
      ...
      validator.balance = validator.balance + deposit;
    }
  ...
  private int get_fork_version(ForkData fork_data, int slot) {
    if (slot < fork_data.fork_slot.getValue()) {
      return toIntExact(fork_data.pre_fork_version.getValue());
    } else {
      return toIntExact(fork_data.post_fork_version.getValue());
    }
  }
